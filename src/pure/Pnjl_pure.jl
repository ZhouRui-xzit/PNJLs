...
@inline function thand(p, phi, Phi1, Phi2, T, muB)
...
# -------------------- 真空项（单段） --------------------
@inline function _integrate_vac_minimal(phi; Λf::Real=Lambda_f)
...
# -------------------- 热项（窗口分段 + 固定网格） --------------------
function _integrate_th_minimal(phi, Phi1, Phi2, T, mu_B;
                               pmax::Real=PMAX_TH,
                               k::Real=K_WIN, delta_min::Real=DELTA_MIN,
                               tol::Real=EPS_MERGE)
    segs  = _make_th_segments(phi, T, mu_B; pmax=pmax, k=k, delta_min=delta_min, tol=tol)
    total = zero(promote_type(eltype(phi), typeof(T), typeof(mu_B)))
    @inbounds for (a,b,iswin) in segs
        total += _gl_sum_segment(a, b, iswin, p -> thand(p, phi, Phi1, Phi2, T, mu_B))
    end
    return total
end
# ============================== Omega ==============================
function Omega(phi, Phi1, Phi2, T, mu_B;
...
# =========================== dOmega 导数 ===========================
function dOmega_dphi(phi, Phi1, Phi2, T, mu_B)
    return ForwardDiff.gradient(x -> Omega(x, Phi1, Phi2, T, mu_B), phi)
end

function dOmega_dPhi1(phi, Phi1, Phi2, T, mu_B)
    return ForwardDiff.derivative(x -> Omega(phi, x, Phi2, T, mu_B), Phi1)
end

function dOmega_dPhi2(phi, Phi1, Phi2, T, mu_B)
    return ForwardDiff.derivative(x -> Omega(phi, Phi1, x, T, mu_B), Phi2)
end

function dOmega_dT(phi, Phi1, Phi2, T, mu_B)
    return ForwardDiff.derivative(x -> Omega(phi, Phi1, Phi2, x, mu_B), T)
end

function dOmega_dmu_B(phi, Phi1, Phi2, T, mu_B)
    return ForwardDiff.derivative(x -> Omega(phi, Phi1, Phi2, T, x), mu_B)
end

# =========================== 方程与求解 ===========================
function function_Quark_core(x_array, T, mu_B)
    # 核心 5 维驻点条件
    phi_u, phi_d, phi_s, Phi1, Phi2 = x_array
    phi = [phi_u, phi_d, phi_s]
    dphi  = dOmega_dphi(phi, Phi1, Phi2, T, mu_B)
    dP1   = dOmega_dPhi1(phi, Phi1, Phi2, T, mu_B)
    dP2   = dOmega_dPhi2(phi, Phi1, Phi2, T, mu_B)
    return [dphi[1], dphi[2], dphi[3], dP1, dP2]
end

function Quark_mu(x_array, T, mu_B)
    # T-mu_B 模式：5 维非线性方程
    T_out = promote_type(eltype(x_array), typeof(T), typeof(mu_B))
    fvec = zeros(T_out, 5)
    fvec[1], fvec[2], fvec[3], fvec[4], fvec[5] = function_Quark_core(x_array, T, mu_B)
    return fvec
end

function Quark_rho(x_array, T, rho_B)
    ...
    mu_B = x_array[6] * 3
    fvec[1:5] .= function_Quark_core(X0, T, mu_B)

    rho_now = -1 .* dOmega_dmu_B(phi, Phi1, Phi2, T, mu_B)
...
function Trho(T, rho_B, X0)
    # 给定 T, rho_B 求解 8 维驻点
    fWrapper(Xs) = Quark_rho(Xs, T, rho_B)
    res = nlsolve(fWrapper, X0)
    return res.zero
end
