...
function calculate_vacuum_term(p, mass)
    E = sqrt.(p.^2 .+ mass^2)
...
function calculate_thermal_term(p, mass, T, mu, Phi1, Phi2)
...
function Omega(phi, Phi1, Phi2, T, mu_B)
   ...
   # p2, w2 = ints[2]  # therm nodes
    ...
    for flavor = 1:3
        ...
        vacuum_contrib = p -> (p^2 / (2*pi^2)) * sqrt(p^2 + mass^2)

 
        # 换元：p = -log(1-x), x in [0,1)
        thermal_contrib_01 = x -> begin
            p   = -log1p(-x)          # = -log(1-x)，数值更稳定
            jac = 1.0 / (1.0 - x)     # dp/dx

            E = sqrt(p^2 + mass^2)
            (p^2 / (2*pi^2)) * jac * T *
                (log(AA(E - mu, T, Phi1, Phi2)) + log(AAbar(E + mu, T, Phi1, Phi2)))
        end

        vac_integral,  _ = quadgk(vacuum_contrib, 0.0, Lambda_f; rtol=1e-8)
        xhi = 1.0 - 1e-12  # 避免 x=1 的奇点
        ther_integral, _ = quadgk(thermal_contrib_01, 0.0, xhi; rtol=1e-8)


        Omega_total += -2 * Nc * vac_integral - 2 * ther_integral
    end
